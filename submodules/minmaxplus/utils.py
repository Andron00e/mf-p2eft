...
def to_weight_regularization(network):
    weights = [
        layer.get_weight_regularizer()
        for layer in network.modules()
        if getattr(layer, "get_weight_regularizer", None)
    ]
    return weights
...
def to_cpu_weight_activation(network):
    ...
    weights = [
        ...
        for layer in layers
        if getattr(layer, "get_weight_activation", None)
        and layer.get_weight_activation() is not None
    ]
    return weights
...
def to_weights(network):
    layers = [
        ...
        for module in network.modules()
        if not isinstance(module, torch.nn.Sequential)
        and not isinstance(module, torch.nn.Flatten)
    ]
...
def reset_weight_activation(network):
    ...
    weight_activation_layers = [
        ...
        for layer in layers
        if getattr(layer, "reset_weight_activation", None)
        and layer.reset_weight_activation() is not None
    ]
